...
export const mockMulter = {
  single: jest.fn((fieldName: string) => {
    return jest.fn(
      (req: MockRequest, res: MockResponse, next: MockNextFunction) => {
        if (fieldName === "avatar") {
          req.file = {
            fieldname: "avatar",
...
export const mockFileSystem = {
  ...
  readFileSync: jest
    .fn<() => Buffer>()
    .mockReturnValue(Buffer.from("test file content")),
...
export const mockCloudStorage = {
  ...
  getFileUrl: jest
    .fn()
    .mockReturnValue("https://mock-cloud-storage.com/files/test-file-123.jpg"),
};
