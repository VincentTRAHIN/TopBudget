...
describe('StatistiquesService', () => {
  ...
  describe('getCategoriesEnHausse', () => {
    it('should identify categories with significant increases', async () => {
      ...
      const categoriesEnHausse = await StatistiquesService.getCategoriesEnHausse(
        testUserId,
        currentStart,
        currentEnd,
        20,
    ...
    // EDGE CASE: Category with zero spending in previous month
    it('should handle zero spending in previous month correctly', async () => {
      const currentStart = new Date('2024-02-01');
      const currentEnd = new Date('2024-02-29');

      // No expenses in previous month (January)
      ...
      expect(categoriesEnHausse[0]).toEqual({
        ...
        totalMoisActuel: 150,
        totalMoisPrecedent: 0,
        variationPourcent: 100, // 100% because previous was 0
        variationValeur: 150
      });
    });

    // EDGE CASE: High percentage but low absolute value increase
    it('should filter out high percentage increases with low absolute value', async () => {
      ...
      const categoriesEnHausse = await StatistiquesService.getCategoriesEnHausse(
        ...
        currentStart,
        currentEnd,
        50, // 50% minimum - this would be met (200% increase)
        10  // 10€ minimum value - this would NOT be met (only 2€ increase)
      );

      expect(categoriesEnHausse).toHaveLength(0);
    });

    // EDGE CASE: Empty data scenario - no transactions at all
    ...
    it('should sort categories by variation percentage descending', async () => {
      ...
      expect(categoriesEnHausse[0].variationPourcent).toBeGreaterThan(categoriesEnHausse[1].variationPourcent);
  ...
  // EDGE CASE TESTS: Empty/Zero Data Scenarios
  describe('Edge Cases - Empty/Zero Data', () => {
    ...
    describe('getRepartitionParCategorie with empty data', () => {
      ...
      it('should handle transactions outside the period gracefully', async () => {
        ...
        // Create transactions outside the query period
        await DepenseModel.create({
          montant: 100,
          date: new Date('2024-01-15'),
          categorie: testCategorieId,
    ...
    describe('getCategoriesEnHausse with empty data', () => {
      ...
      it('should handle user with no transactions gracefully', async () => {
        ...
        // Create transactions for a different user
        ...
        const categoriesEnHausse = await StatistiquesService.getCategoriesEnHausse(
          testUserId, // Query for testUserId who has no transactions
          currentStart,
          currentEnd,
          20,
          10
