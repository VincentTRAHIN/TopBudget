...
/**
 * @swagger
 * /api/depenses/{id}/toggle-charge-fixe:
 *   patch:
 *     tags: [Dépenses]
 *     summary: Basculer le statut charge fixe d'une dépense
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 *     responses:
 *       200:
 *         description: Statut charge fixe modifié avec succès
 *       404:
 *         description: Dépense non trouvée
 *       403:
 *         description: Non autorisé à modifier cette dépense
 */
export const toggleChargeFixe = createAsyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
      return next(new AppError(AUTH.ERRORS.UNAUTHORIZED, 401));
    }
    ...
    try {
      const depense = await DepenseService.toggleChargeFixe(
        req.params.id,
        req.user.id,
      );
      return sendSuccess(res, 'Statut charge fixe modifié avec succès', depense);
    } catch (error) {
      logger.error('Erreur lors du toggle charge fixe', error);
      next(error);
    }
  },
);
