...
test.describe("Statistiques Page", () => {
  ...
  test("should display the Statistiques page with title and context switcher", async ({ authenticatedPage }) => {
    ...
    const contextSwitcher = authenticatedPage.locator(
      'button:has-text("Mes Statistiques"), button:has-text("Statistiques du Couple")',
    );
  ...
  test("CategoryBreakdown - should display top 5 expense categories with progress bars", async ({
    authenticatedPage,
  }) => {
    // Attendre le chargement des données
    await authenticatedPage.waitForTimeout(2000);
    ...
    const progressBars = authenticatedPage.locator(
      '[class*="bg-red-500"], [class*="bg-blue-500"], [class*="bg-green-500"]',
    );
  ...
  test("Statistiques - should handle empty data states gracefully", async ({ authenticatedPage }) => {
    ...
    const hasContent =
      bodyText && (bodyText.includes("€") || bodyText.includes("Aucun") || bodyText.includes("Pas de"));
