...
const mockUseExpensesTrends = useExpensesTrends as jest.MockedFunction<
  typeof useExpensesTrends
>;
...
describe('ExpensesTrendsChart Component', () => {
  ...
  describe('États de chargement et d\'erreur', () => {
    ...
    it('should display error message when isError is true', () => {
      ...
      expect(
        screen.getByText('Impossible de récupérer les tendances de dépenses')
      ).toBeInTheDocument();
    });
    ...
    it('should display empty state when no data is available', () => {
      ...
      expect(
        screen.getByText('Aucune donnée disponible pour la période sélectionnée')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Props et configuration', () => {
    ...
    it('should display correct subtitle with nbMois', () => {
      ...
      expect(
        screen.getByText('Évolution des top 5 catégories sur 12 mois')
      ).toBeInTheDocument();
    });
  });
