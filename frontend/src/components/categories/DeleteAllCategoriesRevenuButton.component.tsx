...
interface DeleteAllCategoriesRevenuButtonProps {
  onSuccess: () => void;
}

export default function DeleteAllCategoriesRevenuButton({ onSuccess }: DeleteAllCategoriesRevenuButtonProps) {
  ...
  const handleDeleteAll = async () => {
    setIsDeleting(true);
    try {
      const response = await fetcher(`${categoriesRevenuEndpoint}/delete-all`, {
        method: "DELETE",
      });

      const data = response as { deletedCount: number };
      toast.success(`${data.deletedCount} catégorie(s) de revenu supprimée(s) avec succès`);
      setShowConfirm(false);
      onSuccess();
    } catch (error) {
      toast.error("Erreur lors de la suppression des catégories de revenu");
      console.error(error);
    } finally {
      setIsDeleting(false);
    }
            ...
            <div className="mt-3 text-center">
              ...
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  Êtes-vous sûr de vouloir supprimer toutes vos catégories de revenu ?
                  <br />
