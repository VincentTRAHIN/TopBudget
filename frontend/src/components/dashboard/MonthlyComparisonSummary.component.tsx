...
const MonthlyComparisonSummary: React.FC<{ statsContext?: 'moi' | 'couple' }> = ({ statsContext = 'moi' }) => {
  ...
  if (isError) {
    return <div className="bg-white p-6 rounded-lg shadow-md text-red-500">Erreur lors du chargement des données.</div>;
  }
  ...
  const trendColor = difference === 0 ? "text-gray-600" : difference > 0 ? "text-red-600" : "text-green-600";
  const trendBg = difference === 0 ? "bg-gray-50" : difference > 0 ? "bg-red-50" : "bg-green-50";
  const trendText = difference === 0 ? "Dépenses stables" : difference > 0 ? "Augmentation des dépenses" : "Réduction des dépenses";
  ...
  const prevMonth = new Date(new Date().setMonth(new Date().getMonth() - 1)).toLocaleDateString('fr-FR', { month: 'long' });
