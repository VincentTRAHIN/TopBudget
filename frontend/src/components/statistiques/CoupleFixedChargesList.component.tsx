...
export default function CoupleFixedChargesList() {
  ...
  // Rendu principal si data et data.chargesFixes existent
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200 mb-4">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          {data.chargesFixes.map(charge => (
            <tr key={charge._id || `${charge.description}-${charge.montant}-${charge.payePar}`}> 
              <td className="px-4 py-2">{charge.description}</td>
              <td className="px-4 py-2">{charge.montant.toFixed(2)}€</td>
              <td className="px-4 py-2">{typeof charge.categorie === 'string' ? charge.categorie : (charge.categorie?.nom || 'N/A')}</td>
