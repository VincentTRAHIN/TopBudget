...
export default function StatsSummary() {
  ...
  useEffect(() => {
    if (!evolutionLoading && evolutionData && evolutionData.length > 0) {
      const totalExpenses = evolutionData.reduce((sum, item) => sum + item.totalDepenses, 0);
      ...
      evolutionData.forEach(item => {
        if (item.totalDepenses < minExpense.value) {
          minExpense = { 
            ...
            month: new Date(item.mois + '-01').toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })
          };
        }
      ...
      if (lastThreeMonths.length >= 2) {
        ...
        const lastValue = lastThreeMonths[lastThreeMonths.length - 1].totalDepenses;
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded-lg p-4 bg-blue-50">
          ...
          <p className="text-gray-700">Moyenne mensuelle: <span className="font-bold">{stats.average.toFixed(2)}€</span></p>
          <p className="text-gray-700">Mois en cours: <span className="font-bold">{currentMonthTotal.toFixed(2)}€</span></p>
          <p className="text-gray-700 text-sm mt-1">
            {currentMonthTotal > stats.average ? 
              <span className="text-red-500 flex items-center">
                <ArrowUpRight size={16} className="inline mr-1" /> 
                {((currentMonthTotal - stats.average) / stats.average * 100).toFixed(1)}% au-dessus de la moyenne
              </span> : 
              <span className="text-green-500 flex items-center">
                <ArrowDownRight size={16} className="inline mr-1" /> 
                {((stats.average - currentMonthTotal) / stats.average * 100).toFixed(1)}% en-dessous de la moyenne
              </span>
            }
          </p>
        </div>
        ...
        <div className="border rounded-lg p-4 bg-green-50">
          ...
          <p className="text-gray-700">
            Maximum: <span className="font-bold">{stats.maximum.value.toFixed(2)}€</span> 
            <span className="text-sm ml-1">({stats.maximum.month})</span>
          </p>
          <p className="text-gray-700">
            Minimum: <span className="font-bold">{stats.minimum.value.toFixed(2)}€</span> 
            <span className="text-sm ml-1">({stats.minimum.month})</span>
          </p>
          <p className="text-gray-700 text-sm mt-1">
            {stats.trend === 'up' ? (
              <span className="text-red-500">Tendance à la hausse des dépenses</span>
            ) : stats.trend === 'down' ? (
              <span className="text-green-500">Tendance à la baisse des dépenses</span>
            ) : (
              <span className="text-gray-500">Dépenses stables sur les derniers mois</span>
            )}
          </p>
        </div>
      </div>
