...
export default function ExpensesTrendsChart({ contexte = "moi", nbMois = 6 }: ExpensesTrendsChartProps) {
  const { evolutionMensuelle, topCategories, isLoading, isError } = useExpensesTrends(contexte, nbMois);
    ...
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="text-center text-red-600">
          ...
          <p className="text-sm mt-1">Impossible de récupérer les tendances de dépenses</p>
        </div>
      </div>
    );
  }
  ...
  if (!evolutionMensuelle || evolutionMensuelle.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Tendances de Dépenses</h3>
  ...
  const datasets = top5Categories.map((category, index) => {
    const data = evolutionMensuelle.map((mois) => {
      const catData = mois.categories.find((c) => c.categorieId === category.categorieId);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      {/* En-tête */}
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-900">Tendances de Dépenses</h3>
        <p className="text-sm text-gray-600 mt-1">Évolution des top 5 catégories sur {nbMois} mois</p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {top5Categories.map((category, index) => {
          ...
          return (
            <div key={category.categorieId} className="flex items-center space-x-3 p-3 rounded-lg bg-gray-50">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: color }}></div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">{category.nom}</p>
                <p className="text-xs text-gray-500">{category.totalActuel.toFixed(2)} €</p>
              </div>
              <div className={`flex items-center ${tendanceColor}`}>
                <TrendIcon className="w-4 h-4" />
                <span className="text-xs font-medium ml-1">{Math.abs(category.variationPourcent).toFixed(0)}%</span>
              </div>
            </div>
          );
        })}
