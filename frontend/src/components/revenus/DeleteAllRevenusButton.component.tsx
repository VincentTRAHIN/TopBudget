/**
 * Composant bouton pour supprimer tous les revenus avec confirmation modale
 */
import { useDeleteAllRevenus } from "@/hooks/useDeleteAllRevenus.hook";
import React, { useState } from "react";
import toast from "react-hot-toast";

export const DeleteAllRevenusButton: React.FC = () => {
  const [showModal, setShowModal] = useState(false);
  const { deleteAllRevenus, isLoading } = useDeleteAllRevenus();

  const handleDelete = async () => {
    try {
      const result = await deleteAllRevenus();

      setShowModal(false);
      toast.success(`${result.deletedCount} revenus supprimés avec succès`);
    } catch (error) {
      console.error("Erreur lors de la suppression des revenus:", error);
      toast.error("Erreur lors de la suppression des revenus");
    }
  };
            ...
            <div className="mt-3 text-center">
              ...
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  Êtes-vous sûr de vouloir supprimer tous vos revenus ?
                  <br />
