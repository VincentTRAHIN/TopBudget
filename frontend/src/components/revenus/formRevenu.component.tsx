...
export default function FormRevenu({ existingRevenu, onClose }: { existingRevenu?: IRevenu; onClose?: () => void }) {
  ...
  const initialValues: RevenuFormValues = {
    montant: existingRevenu?.montant || "",
    description: existingRevenu?.description || "",
    date: existingRevenu?.date.slice(0, 10) || new Date().toISOString().slice(0, 10),
    typeCompte: TypeRevenuEnum[existingRevenu?.typeCompte as keyof typeof TypeRevenuEnum] || "Perso",
