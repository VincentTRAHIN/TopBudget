...
export function useColumns({ currentUserId, onEdit, refreshDepenses }: UseColumnsProps) {
  const log = debug("app:frontend:useColumnTableDepenses");

  const handleToggleChargeFixe = useCallback(
    async (depenseId: string, currentValue: boolean) => {
      try {
          ...
          async (currentData) => {
            ...
            return {
              ...
              depenses: currentData.depenses.map((depense) =>
                depense._id === depenseId ? { ...depense, estChargeFixe: !currentValue } : depense,
              ),
            };
          },
          {
        ...
        toast.success(!currentValue ? "🔒 Dépense marquée comme charge fixe" : "🔓 Dépense retirée des charges fixes");
        ...
        return (
          <button
            ...
            className={`
              ...
              ${isFixed ? "bg-blue-100 text-blue-800 hover:bg-blue-200" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}
            `}
            ...
            <span className="text-xs font-medium">{isFixed ? "Fixe" : "Variable"}</span>
          </button>
        ) as any;
      },
