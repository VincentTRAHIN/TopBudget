...
export default function FormDepense({
  ...
  return (
    <div key={existingDepense?._id || "new"} className="bg-white p-6 rounded-lg shadow-md mb-6 relative">
      ...
      <h3 className="text-lg font-semibold mb-4">{existingDepense ? "Modifier la Dépense" : "Ajouter une Dépense"}</h3>

      <Formik initialValues={initialValues} validationSchema={DepenseSchema} onSubmit={handleSubmit} enableReinitialize>
        {({ isSubmitting }) => (
