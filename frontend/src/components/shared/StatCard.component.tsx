...
import { FinancialStats, calculatePercentage, formatMonthYear } from "@/utils/stats.utils";
import { Activity, ArrowDownRight, ArrowUpRight, HelpCircle, LucideIcon } from "lucide-react";
    ...
    <div className={`border rounded-lg p-4 ${bgColor}`}>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center">
          ...
          <h4 className={`text-md font-medium ${iconColor.replace("text-", "text-")}`}>{title}</h4>
        </div>
        <Tooltip
          isOpen={tooltipOpen}
      ...
      <p className="text-gray-700">
        Moyenne mensuelle: <span className="font-bold">{stats.average.toFixed(2)}€</span>
        {!stats.hasEnoughData && <span className="text-xs text-gray-500 ml-1">(basée sur le mois en cours)</span>}
      </p>

      {/* Mois en cours */}
      <p className="text-gray-700">
        Mois en cours: <span className="font-bold">{currentValue.toFixed(2)}€</span>
      </p>
        ...
        {stats.hasEnoughData ? (
          isAboveAverage ? (
            <span className={`${comparisonColor} flex items-center`}>
              ...
              {calculatePercentage(currentValue, stats.average)} au-dessus de la moyenne
            </span>
          ) : (
            <span className={`${comparisonColor} flex items-center`}>
              <ArrowDownRight size={16} className="inline mr-1" />
              {calculatePercentage(currentValue, stats.average)} en-dessous de la moyenne
            </span>
          )
        ) : (
      ...
      <p className="text-gray-700">
        ...
        <span className="text-sm ml-1">
          (
          {typeof stats.maximum.month === "string" && stats.maximum.month !== "Mois en cours"
      ...
      <p className="text-gray-700">
        ...
        <span className="text-sm ml-1">
          (
          {typeof stats.minimum.month === "string" && stats.minimum.month !== "Mois en cours"
