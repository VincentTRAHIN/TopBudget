...
export const useCoupleContributionsSummary = (annee: string, mois: string) => {
  ...
  const {
    ...
    mutate,
  } = useSWR<BackendCoupleContributionsResponse>(summaryUrl, fetcher);
  ...
  const {
    ...
    error: commonExpensesError,
    isLoading: isCommonExpensesLoading,
  } = useSWR<CommonExpensesResponse>(commonExpensesUrl, fetcher);

  const isLoading = isSummaryLoading || isCommonExpensesLoading;
  const error = summaryError || commonExpensesError;

  const data: CoupleContributionsSummary | undefined =
    !isLoading && !error && rawSummaryData && commonExpensesData && user?._id
      ? (() => {
          ...
          if (
            commonExpensesData &&
            Array.isArray(commonExpensesData.depenses) &&
            commonExpensesData.depenses.length > 0
          ) {
